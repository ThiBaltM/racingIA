...
class NeuralNetwork:
    def __init__(self, input_size=None, hidden1_size=None, hidden2_size=None, hidden3_size=None, output_size=None, data=None, data2=None):
        if(data2==None and data==None):
            ...
            self.weights3 = [[random.random()*random.choice([1,-1]) for _ in range(hidden2_size)] for _ in range(hidden3_size)]
            self.weights4 = [[random.random()*random.choice([1,-1]) for _ in range(hidden3_size)] for _ in range(output_size)]
            ...
            self.bias3 = [(random.random()*random.choice([0.1,-0.1])) for _ in range(hidden3_size)]
            self.bias4 = [(random.random()*random.choice([0.1,-0.1])) for _ in range(output_size)]
        elif data2==None:
            ...
            self.hidden3_size = parent1["hidden3_size"]
            ...
            self.weights4 = []
            for k in range(len(parent1["weights4"])):
                self.weights4.append([])
                for i in range(len(parent1["weights4"][0])):
                    w1 = parent1["weights4"][k][i]
                    w2 = parent2["weights4"][k][i]
                    if(random.random()<noMutateRate):
                        self.weights4[k].append(random.uniform(w1,w2))
                    else:
                        self.weights4[k].append(random.random())
            ...
            self.bias4 = []
            for k in range(len(parent1["bias4"])):
                b1 = parent1["bias4"][k]
                b2 = parent2["bias4"][k]
                if(random.random()<noMutateRate):
                    self.bias4.append(random.uniform(b1,b2))
                else:
                    self.bias4.append(random.random())
    ...
    def forward(self, X):
        ...
        z6 = [self.sigmoid(z5[j]) for j in range(self.output_size)]
        z7 = self.calcVal(z6, self.weights4, self.bias4, self.output_size)
        o = [self.sigmoid(z7[j]) for j in range(self.output_size)]
        return o
